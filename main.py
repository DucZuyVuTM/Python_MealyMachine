class FiniteStateMachine:
    def __init__(self):
        ...
        self.variables = {'d': 0, 'q': 0, 'u': 0}  # Variables for conditional transitions
...
def test():
    ...
    assert obj.seen_method('stay') == False
    assert obj.seen_method('forge') == False
    
    # Test seen_edge before any transitions
    assert obj.seen_edge('F3', 'F4') == False
    assert obj.seen_edge('F7', 'F5') == False
    
    # Test initial step count
    assert obj.get_step() == 0
    
    # Test unsupported method in current state
    assert obj.select('slur') == 'unsupported'  # slur not available from F7 with d=1
    ...
    assert obj2.seen_method('forge') == True
    assert obj2.seen_edge('F7', 'F5') == True
    
    # Test etch with q=1
    assert obj2.select('etch') == 'H4'  # F5->F2 with q=1
    assert obj2.seen_method('etch') == True
    assert obj2.seen_edge('F5', 'F2') == True
    ...
    assert obj3.seen_edge('F5', 'F3') == True
    
    assert obj3.select('slur') == 'H4'   # F3->F4
    assert obj3.seen_edge('F3', 'F4') == True
    ...
    assert obj3.seen_edge('F0', 'F0') == True
    
    # Test edge that should not have been seen
    assert obj3.seen_edge('F1', 'F5') == False
    assert obj3.seen_edge('F2', 'F3') == False
